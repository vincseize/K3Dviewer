...
class SideNavBar(QFrame):
    ...
    def init_ui(self):
        ...
        # On s'assure que le bouton envoie bien le signal au viewer
    ...
    def _toggle_zoom(self, checked):
        if checked:
            self.btn_pan.blockSignals(True); self.btn_pan.setChecked(False); self.btn_pan.blockSignals(False)
            self.viewer.activate_zoom_mode(True)
        else: self.viewer.activate_zoom_mode(False)

    def _toggle_pan(self, checked):
        if checked:
            self.btn_zoom.blockSignals(True); self.btn_zoom.setChecked(False); self.btn_zoom.blockSignals(False)
            self.viewer.activate_pan_mode(True)
        else: self.viewer.activate_pan_mode(False)

    def uncheck_all_modes(self):
        self.btn_zoom.blockSignals(True); self.btn_pan.blockSignals(True)
        self.btn_zoom.setChecked(False); self.btn_pan.setChecked(False)
        self.btn_zoom.blockSignals(False); self.btn_pan.blockSignals(False)